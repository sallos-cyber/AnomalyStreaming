...
def streamingFromFileWithAggregationToConsole():
    ...
    avgSignalDF.createOrReplaceTempView("flows")
    anotherStreamingDF = spark.sql("select count(*) from flows")  # returns another streaming DF




    #win = avgSignalDF.select('window')
    #print(avgSignalDF.isStreaming)
    print(avgSignalDF.printSchema())

    print('is this widowedCounts streaming?')
    print('if here is no output you need to delete contents from the checkpoint location here sscd\/tmp')
    print(avgSignalDF.isStreaming)
